...
namespace HW4AzureFunctions
{
    ...
    {
        const string ImagesToConvertRoute = "convertedimages/{name}";

        //ImageStatusUpdaterSuccess/{name}
        ...
        public static async void Run([BlobTrigger(ImagesToConvertRoute, Connection = ConfigSettings.STORAGE_CONNECTIONSTRING_NAME)] CloudBlockBlob cloudBlockBlob, string name, ILogger log)
        {
            ...
            /*
             * This azure function will be triggered when images are uploaded into the convertedimages container. Update the job with the status of 4. Also update the
            ...
            await UpdateJobTableWithStatus(log, jobId, "3", "Image successfully converted", cloudBlockBlob.Uri.AbsolutePath);
