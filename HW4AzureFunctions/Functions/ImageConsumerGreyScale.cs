...
namespace HW4AzureFunctions
{
    ...
    public static class ImageConsumerGreyScale
    {
        ...
        private const string ConversionType = "GreyScale";
        ...
        public static async void Run([BlobTrigger(ImagesToConvertRoute, Connection = ConfigSettings.STORAGE_CONNECTIONSTRING_NAME)] CloudBlockBlob cloudBlockBlob, string name, ILogger log)
        {
            ...
            //This azure function will be triggered when images are uploaded into the converttogreyscale container. It will then write out a record to the imageconversionjobs table with a
            //status of 1 and an imageConversionMode of GreyScale.It will then convert the image to a grey scale image and store the resultant image as a blob in the convertedimages
            //container if the conversion was a success or the failedimages container if conversion failed.When your code enters the stage where it's about to convert the image,
            //change the status in the jobs table to 2. When initially setting or updating the status ensure that you also update the statusDescription with the human readable
            //description of the status as defined in the job status table definition.
                                                 ...
                                                 CloudBlobContainer failedImagesContainer)
        {
            try
            {
                ...
                using (Image<Rgba32> image = (Image<Rgba32>)Image.Load(uploadedImage))
                {
                    ...
                    CloudBlockBlob convertedBlockBlob = convertedImagesContainer.GetBlockBlobReference(_convertedBlobName);
                    ...
                    log.LogInformation($"[-] Stored converted image {_convertedBlobName} into {ConfigSettings.CONVERTED_IMAGES_CONTAINERNAME} container");
            ...
            catch (Exception ex)
            {
                ...
                await StoreFailedImage(log, uploadedImage, blobName, failedImagesContainer, convertedBlobName: _convertedBlobName, jobId: _jobId);
            }
        }
    }
}
